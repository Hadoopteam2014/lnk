...
CONSUMER_KEY = '755qpuiqzxjcs1'
CONSUMER_SECRET = 'BC4avPVKmEzjsbgX'
USER_TOKEN = '485ad139-bdd5-41a6-9d9d-91964731ca8b'
USER_SECRET = 'f201a654-d52c-43f7-bfd2-0bbff2a69c1d'
...
x = app.get_profile(selectors=['id', 'firstName', 'lastName', 'location', 'numConnections', 'skills', 'educations'])
y = project(firstname = x['firstName'], lastname = x['lastName'], ids = x['id'], location = x['location'], skills = x['skills'],numconnections = x['numConnections'], educations = x['educations'])

for each in x:
    ## The result is also in a tuple
    for y in each:
        ## Unpickle the stored string
        z = project(firstname = x['firstName'], lastname = x['lastName'], ids = x['id'], location = x['location'], skills = x['skill'], 
       numconnections = x['numConnections'], educations = x['educations'])
        print z
